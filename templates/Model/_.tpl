<?php
 ...
 class {{Module}} extends AbstractModel implements {{Module}}Interface, IdentityInterface {
    
  public const CACHE_TAG = '{{module}}_b';
  ...
  protected function _construct() {
   $this->_init(\{{Vendor}}\{{Module}}\Model\ResourceModel\{{Module}}::class);
  }
  ...
  public function beforeSave() {
   ...
   $needle = '{{module}}_id="' . $this->getId() . '"';
   $content = ($this->getContent() !== null) ? $this->getContent() : '';
   if(strpos($content, $needle) !== false) {
    throw new \Magento\Framework\Exception\LocalizedException(
     __('Make sure that {{module}} content does not reference the {{module}} itself.')
    );
   }
  ...
  public function getId() {
   return $this->getData(self::{{MODULE}}_ID);
  }
  ...
  public function setId($id) {
   return $this->setData(self::{{MODULE}}_ID, $id);
  }
